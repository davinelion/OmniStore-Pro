# Obtainium Features in OmniStore Pro

OmniStore Pro now includes **Obtainium's best features** — direct app updates from source, background checks, and full user control. Obtainium is an Android app that gets updates directly from GitHub releases, etc. We brought its philosophy to the web for all platforms.

## Why Obtainium?

Traditional app stores (Play Store, App Store) are middlemen. Obtainium bypasses them:
...
- Tracked apps appear in **/updates** — Obtainium-style update center
...
2. Clicks Track
3. OmniStore remembers installed version (user can set it, like Obtainium reads from device)
...
- **Global settings** (`GlobalUpdateSettings` in IndexedDB):
  - `checkIntervalHours`: 1, 3, 6 (Obtainium default), 12, 24
...
### 3. Per-App Settings (Obtainium)

Each tracked app has:

- `autoUpdate`: Auto-update when new version (Obtainium background update per-app)
- `includePrerelease`: Include RC/beta (Obtainium allow pre-release)
- `trackOnly`: Don't show in updates, just track (Obtainium track-only)
- `allowDowngrade`: Show previous releases for rollback (Obtainium rollback)
- `wifiOnly`, `chargingOnly`: Per-app overrides
- `skippedVersion`: Skip a version — don't notify (Obtainium skip version)
- `installedVersion`: What user has installed — like Obtainium reads from device. Updates calculated from this.
- `versionFilter`: Custom regex (future, stored)
- `lastNotifiedAt`: When last notified

UI in `ObtainiumCard`: gear icon → settings panel with toggles, installed version input, skip/unskip, stop tracking.
...
- Like Obtainium's import/export app list, share configurations
...
- User can set installed version manually (input field) — like Obtainium reads version from device
...
- Same as Obtainium: GitHub, GitLab, Codeberg, Forgejo, F-Droid, Flathub, Winget, Homebrew, Other
...
- `/updates` — **main Obtainium center**: search, filter (all/updates/up-to-date), batch actions, global settings, import/export
...
- All downloads go to upstream (GitHub) — never mirrored, like Obtainium

## UI Components

- `src/lib/track/types.ts` — Enhanced TrackedApp with Obtainium fields + `defaultTrackedSettings()`
...
- `src/components/updates/ObtainiumCard.tsx` — Card per tracked app: installed vs latest, direct download, assets preview, per-app settings
...
4. **Settings** — set check interval to 6h (Obtainium default), enable notifications
...
## Comparison with Obtainium

| Feature | Obtainium (Android) | OmniStore Pro |
|---------|-------------------|---------------|
...
OmniStore Pro is **Obtainium + App Store + F-Droid + AI** — best of all.
...
- Version regex filter UI (Obtainium custom HTML source)
- Auto-install via PWA File Handling + Native File System (when browser allows)
- Share tracked list via URL (like Obtainium share config)
