/** GET /api/v1/recommendations — OmniSource recommendation response. */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const dto = await getOmnisource().request(
      `/recommendations?app_id=${encodeURIComponent(appId)}&limit=${intParam(params.get("limit"), 12, 1, 100)}`,
      RecommendationResponseSchema,
      { tags: ["recommendations", `app:${appId}`], revalidate: 600 },
    );
    return ok(dto);
  } catch (error) {
    return fail(error);
